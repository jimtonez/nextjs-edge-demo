...
import { teams } from '@/constants/nhl'
    ...
    return (
        <section className="grid grid-cols-1 md:grid-cols-2 md:max-w-3xl lg:grid-cols-3 lg:max-w-5xl xl:grid-cols-4 xl:max-w-6xl mx-auto gap-4 px-2 lg:px-4">
                ...
                {teams.map((team, index) => (
                    <Link href={{ pathname: `/leagues/nhl/teams/${encodeURIComponent(team.id)}`, query: { name: team.name, logo: team.logo } }} key={team.id} className='flex col-span-1 aspect-w-3 aspect-h-2 cursor-pointer'>
