...
function ImageBio ( image: Image) {
    ...
    const renderImage = () => {
        if (image.name === "nextjs"){
            return (
                <Image priority className={`${styles.logo} p-4`} src="/next.svg" alt="Next.js Logo" width={180} height={37}/>
            )
        } else if (image.name=== "quarkus") {
            return (
                <Image priority className={`mb-3 p-4`} src="/runtimes.svg" alt="Next.js Logo" width={180} height={37}/>
            )
        } else if (image.name === "postgres") {
            return (
                <Image priority className={`mb-3 p-1`} src="/postgres.png" alt="Next.js Logo" width={65} height={37}/>
            )
    ...
    return (
        ...
        <div className="flex flex-col w-full h-full space-y-4">
            <div className="flex flex-col md:flex-row w-full h-[500px] md:h-[350px] lg:h-[500px] items-center justify-center space-y-2 md:space-y-0 md:space-x-4">
                <div className='flex flex-col w-full md:w-1/2 h-3/4 md:h-full items-center justify-center border border-gray-700 border-dashed rounded-xl'>
                    <div className="flex h-1/4 w-full items-center justify-center animate-pulse">
                        {renderImage()}
                    </div>
